...
(function () {
  ...
  // injizieren — so bleibt index.html/QC unangetastet (Mixarium md5-Regel).
  ...
  function mountRendezvousUI() {
    ...
    try {
      window.addEventListener("sbkim:geraetename-changed", function () {
        ...
        try { if (window.SbkimRendezvous && window.SbkimRendezvous.configure) window.SbkimRendezvous.configure({ nodeName: displayNodeName("Mein Mixarium") }); } catch (_e) {}
      });
    } catch (_e) {}
  }
