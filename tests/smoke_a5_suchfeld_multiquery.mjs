...
// von Mein Mixarium (Funktion `semRun`, Gratis-Pfad). Run:
//   node tests/smoke_a5_suchfeld_multiquery.mjs
//
// Der bestehende smoke_rollout_a1a4.mjs beweist die API-Kette im Cross-Knoten-
// ANTWORT-Pfad (15_membran). Dieser Test beweist dieselbe Kette im SUCHFELD und
// nutzt dabei die TATSÄCHLICH AUSGELIEFERTE Synonym-Karte `SEM_SYN` aus
// index.html (mini Drift-Guard der Verdrahtung):
//   expandQuerySimple(term, {synonyms: SEM_SYN}) → queryLocalMulti(vars, k,
...
// Ausgeliefertes Modul 04 (byte-1:1 aus Sage) laden.
...
// SEM_SYN aus der AUSGELIEFERTEN index.html extrahieren — so prüft der Test die
// echte, versendete Karte (nicht eine Test-Kopie). Bricht der Rollout die Karte,
// bricht dieser Test.
...
// Drinks-Korpus: der gesuchte Drink trägt „drink" im text, aber NICHT das
// Query-Token „cocktail". passageVec = e1 (orthogonal zum Query e0 → Cosinus 0).
// Nur die Multi-Query-Variante „drink" kann ihn über BM25 aufnehmen.
const CORPUS = [
  { label: "Waldbeeren-Mix", text: "fruchtiger drink mit waldbeeren, eiswürfel, minze", passageVec: unit(1), anchorId: "mx-1" },
...
async function run() {
  ...
  check("cocktail → drink verdrahtet", Array.isArray(SEM_SYN.cocktail) && SEM_SYN.cocktail.includes("drink"));

  console.log("\nProbe 1 — expandQuerySimple mit SEM_SYN fächert auf");
  const vars = M.expandQuerySimple("cocktail", { synonyms: SEM_SYN });
  check("Original zuerst", vars[0] === "cocktail");
  check("Variante 'drink' erzeugt", vars.includes("drink"));

  console.log("\nProbe 2 — Single-Query verpasst den Cross-Phrasing-Treffer");
  const single = await M.queryLocal("cocktail", 5, { corpus: CORPUS, hybrid: true });
  const singleIds = new Set(single.map((h) => h.anchorId));
  check("Single-Query 'cocktail' findet 'drink'-Passage NICHT (Cosinus 0, kein Token)", !singleIds.has("mx-1"));

  console.log("\nProbe 3 — Multi-Query (Suchfeld-Verdrahtung) nimmt ihn AUF");
  const multi = await M.queryLocalMulti(vars, 5, { corpus: CORPUS, hybrid: true });
  const multiIds = new Set(multi.map((h) => h.anchorId));
  check("Multi-Query nimmt 'drink'-Passage über BM25-Variante auf", multiIds.has("mx-1"));
  check("Ergebnis ist Array (fail-soft, kein Wurf)", Array.isArray(multi));

  console.log("\nProbe 4 — Off-Topic bleibt draußen (kein falsches Aufweichen)");
  check("Wespen-Passage NICHT im Multi-Query-Ergebnis", !multiIds.has("mx-2"));
