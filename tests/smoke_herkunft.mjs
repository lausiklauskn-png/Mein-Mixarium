...
const BESTAND = [
  { id:1, name:"Caipirinha",  cat:"ckt", shut:true, ings:[], steps:[] },
  { id:2, name:"Eistee",    cat:"tee",   shut:true, ings:[], steps:[] },
...
await seite.addInitScript(b=>{ localStorage.setItem("mix9m", JSON.stringify(b)); localStorage.setItem("mxlang9m","de"); }, BESTAND);
...
ok("die Kennung bleibt über den zweiten Export gleich",
   d2.recipes.find(r=>r.name==="Caipirinha").uid === rez1.find(r=>r.name==="Caipirinha").uid);
...
const g3 = d3.recipes.find(r=>r.name==="Caipirinha");
...
ok("Gulasch gibt es genau einmal", nachGleich.filter(r=>r.n==="Caipirinha").length===1);
...
umbenannt.recipes.forEach(r=>{ if(r.name==="Caipirinha") r.name="Caipirinha nach Omas Art"; });
...
ok("es gibt weiterhin genau ein Rezept mit dieser Kennung",
   nachUmbenannt.filter(r=>r.u===nachGleich.find(x=>x.n==="Caipirinha").u).length===1);
...
fremd.recipes = [{ id:77, name:"Caipirinha", uid:"fremder-knoten-abc-12345", cat:"ckt", shut:true, ings:[], steps:[],
...
ok("beide tragen ihre eigene Kennung",
   nachFremd.filter(r=>r.n==="Caipirinha"||r.n==="Caipirinha nach Omas Art").length===2);
...
const alteDatei = { version:9, recipes:[
  { id:1, name:"Caipirinha", cat:"ckt", shut:true, ings:[], steps:[] },      // Name schon da → fällt weg
  { id:2, name:"Ganz-Neues-Gericht", cat:"ckt", shut:true, ings:[], steps:[] } ] };
...
const mitOrdner = { version:10, knoten:"fremd", folders:[{id:"5",name:"FREMD-DARF-NICHT-GEWINNEN",ico:"❌"},{id:"42",name:"Mutti-Ordner",ico:"📂"}],
  recipes:[{ id:9, name:"Aus-Muttis-Ordner", uid:"mutti-xyz-99", cat:"ckt", folder:"42", shut:true, ings:[], steps:[] }] };
...
await seite.evaluate(()=>{
  const f=new File([JSON.stringify({version:10,recipes:[
    {id:1,name:"Sichtbar",cat:"ckt",uid:"a-1"},
