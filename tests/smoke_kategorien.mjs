...
const kz = await seite.evaluate(async () => {
  ...
  /* ⚠ BENANNTE GRENZE (nur hier): die Leiste zaehlt mit `alcAllowed`.
     Steht der Alkohol-Filter an, zeigt sie mit ABSICHT weniger als der
     Bestand — dann verglichen „Leiste" und „echt" zwei verschiedene Fragen. */
  try{ localStorage.setItem("mxNoAlc9","0"); }catch(e){}
  FD = [{ id:"o1", name:"Japanisch" }];
  R = [
    { id:91, name:"Zuordnen-Probe-1", cat:"smooth", folder:"o1", shut:true, ings:[], steps:[] },
  ...
  const ziel = [...pop.querySelectorAll('button')]
    .find(b => (b.textContent||"").includes(katBeschriftung(catsAlle().find(c=>c.id==="smooth"))));
  ziel.click();
  ...
  const zielLeiste = (() => {
    const p = [...document.querySelectorAll('#catNav .cpill')]
      .find(e => (e.getAttribute('onclick')||"").includes("setCAT('smooth')"));
    if (!p) return -1;
    const z = p.querySelector('span');           // die Zahl steht im letzten span
    return z ? +String(z.textContent).trim() : -1;
  })();
  const zielEcht = R.filter(r => r.name && katVonRezept(r) === "smooth").length;
  ...
  [...document.getElementById('katZuPop').querySelectorAll('button')]
    .find(b => (b.textContent||"").includes(katBeschriftung(catsAlle().find(c=>c.id==="smooth")))).click();
...
ok("ein `fld_…`-Altbestand wird ERSETZT, nicht danebengelegt", kz.cat92==="smooth");
...
/* ══ 20 · EIN ORDNER, DEN ES NICHT GIBT, IST KEIN ORDNER (Klaus 2026-09-16) ══
   ...
   Getraenk faellt dann aus JEDER Kategorie-Gruppe (die fragt `!r.folder`) UND es
   gibt keinen Ordner-Eintrag — es bleibt nur die Zahl „+N in Ordnern". */
...
const geist = await seite.evaluate(() => {
  ...
  R = [
    { id:81, name:"Geister-Drink", cat:"", folder:"tot999", shut:true, ings:[], steps:[] },
  ...
  const imBaum = gruppe ? [...gruppe.querySelectorAll('.fld-rrow')]
    .some(e => (e.textContent||"").includes("Geister-Drink")) : false;
...
/* ⚠ DIE ZUSICHERUNG IST SICHTBARKEIT, NICHT EINE ZAHL. „+1 in Ordnern" waere
   auch dann da, wenn das Getraenk nirgends steht — und genau so sah es aus. */
ok("ein Getraenk mit totem Ordner steht im Baum unter „Ohne Kategorie“", geist.imBaum===true);
...
const ordZeile = await seite.evaluate(() => {
  ...
  R = [
    { id:83, name:"Drin",     cat:"ckt", folder:"o1", shut:true, ings:[], steps:[] },
    { id:84, name:"Draussen", cat:"ckt", folder:"",   shut:true, ings:[], steps:[] },
  ];
  renderFolders();
  const g = [...document.querySelectorAll('#fldTree .fld-grp')]
    .find(x => (x.dataset.gid||"") === "cat_ckt");
  const t = g ? (g.querySelector('.fld-cnt')||{}).textContent || "" : "";
  R = JSON.parse(sichR); FD = JSON.parse(sichFD); renderFolders();
  return t;
});
/* ⚠ UND DAS ABZEICHEN WAR DAFUER BLIND — gefunden hat es die Gegenprobe in
   Muttis Rezeptbuch, nicht das Nachdenken. Der vorhandene Waechter misst
   „Leiste = Baum" und fragt das Abzeichen bei einem TOTEN Ordner gar nicht.
   Gemessen wird deshalb genau die Lage, in der beide Fassungen auseinander-
   gehen: die Kategorie hat EIN Getraenk, und das liegt in einem Ordner, den
   es nicht gibt. */
const abzGeist = await seite.evaluate(() => {
  const sichR = JSON.stringify(R), sichFD = JSON.stringify(FD);
  FD = [];
  R = [{ id:85, name:"Nur-Geist", cat:"ckt", folder:"tot999", shut:true, ings:[], steps:[] }];
  badge();
...
const zieh = await seite.evaluate(() => {
  ...
  const ergebnis = {
    ...
    ordGespeichert: (localStorage.getItem('mxcatord9m')||''),
...
ok("… unter dem eigenen Schluessel, der der Rezeptbuecher bleibt leer",
   zieh.ordAndereApp === "");
...
const nachLaden = await seite.evaluate(async () => {
  ...
  const roh = localStorage.getItem('mxcatord9m');
