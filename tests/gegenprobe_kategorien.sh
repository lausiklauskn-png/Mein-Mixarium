...
"katZuSetzen('+rid+',\\'\\')@@@katZuSetzen('+rid+',\\'smooth\\')"
...
"  const fldCats=catsAlle().filter(c=>R.some(r=>!ordnerVonRezept(r)&&katVonRezept(r)===c.id&&r.name)).length;@@@  const fldCats=catsAlle().filter(c=>R.some(r=>!r.folder&&katVonRezept(r)===c.id&&r.name)).length;"
...
fall "der Speicher-Schluessel ist der eines REZEPTBUCHS" "der Rezeptbuecher bleibt leer" \
"CATS_ORD_KEY='mxcatord9m'@@@CATS_ORD_KEY='mrzcatord9'"
