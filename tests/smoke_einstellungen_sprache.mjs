...
const LANG_KEY  = "mxlang9m";
const APP_NAME  = "Mein Mixarium";
/* Die neuen Schluessel dieser App. NUR HIER gibt es `sYouthHead` — den
   Jugendschutz-Kopf; die Rezeptbuecher haben keinen. Und Muttis Rezeptbuch hat
   die Abschnitte Netz und Werkzeuge gar nicht. Drei Apps dieselbe Zusicherung
   behaupten zu lassen waere in einer davon eine Luege. */
const NEUE = ["sMistralLbl","sMistralSub","sNetzHead","sNetzLbl","sNetzSub",
              "sToolsHead","sToolsSuchLbl","sToolsSuchSub","sToolsPinLbl","sToolsPinSub","sOfflineCap","sYouthHead","sAlcLbl","sAlcSub"];
...
const fremd = await seite.evaluate(()=>[...Array(localStorage.length).keys()]
  .map(i=>localStorage.key(i)).filter(k=>/^(mlang9|mlang9m)$/.test(k)));
