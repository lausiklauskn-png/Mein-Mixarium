...
rechts/unten dahinter. Die Folge steht in `mxcatord9m` und überlebt das Neuladen.

⚠ **Der Speicher-Schlüssel ist app-eigen** — die Rezeptbücher (`mrzcatord9`) bleibt leer. Alle drei
...
Thema: wer ihn umbenennt, gibt **jedem** künftigen Getränk ohne Kategorie diesen
...
### ⚠ Die Pillen hatten hier vorher GAR KEINEN Zieh-Weg

In den Rezeptbüchern nehmen `pillDragOver`/`pillDrop` auch eine gezogene
**Karte** entgegen; im Mixarium gab es die Funktionen überhaupt nicht
(gemessen: null Treffer). Sie tragen hier deshalb **nur** den Kategorie-Zug.
Der Name ist trotzdem derselbe wie in den Schwestern — zwei Namen für dieselbe
Sache sind zwei Stellen, die beim nächsten Nachziehen auseinanderlaufen.
...
node tests/smoke_kategorien.mjs           # echter Browser
NUR_ANKER=1 bash tests/gegenprobe_kategorien.sh   # tote Anker in Sekunden
bash tests/gegenprobe_kategorien.sh       # Wegwerf-Kopie (kein Bau-Schritt — `index.html` ist das byte-identische Spiegelbild)
```

Zuletzt gemessen (2026-09-16, nach dem Umsortieren): **150 grün · 0 ROT** ·
Gegenprobe **82 gefangen · 0 durchgerutscht · 0 aus falschem Grund · 0 tote
Anker**. Beide Rückgabewerte **direkt** gelesen; die Prüfsummen des Baums waren
